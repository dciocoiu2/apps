param(
    ...
    [ValidateSet(
        ...
        "SEARCH","M-SEARCH","NOTIFY","SUBSCRIBE","UNSUBSCRIBE","MERGE"
    )]
...
function Show-Help{
    ...
    [Console]::WriteLine(" -HttpMethod : GET,POST,PUT,DELETE,HEAD,OPTIONS,PATCH,TRACE,CONNECT,PROPFIND,PROPPATCH,MKCOL,COPY,MOVE,LOCK,UNLOCK,REPORT,SEARCH,M-SEARCH,NOTIFY,SUBSCRIBE,UNSUBSCRIBE,MERGE")
...
<option>HEAD</option><option>OPTIONS</option><option>PATCH</option><option>TRACE</option>
<option>CONNECT</option><option>PROPFIND</option><option>PROPPATCH</option><option>MKCOL</option>
<option>COPY</option><option>MOVE</option><option>LOCK</option><option>UNLOCK</option>
<option>REPORT</option><option>SEARCH</option><option>M-SEARCH</option><option>NOTIFY</option>
<option>SUBSCRIBE</option><option>UNSUBSCRIBE</option><option>MERGE</option>
</select><br/>
...
function Invoke-HttpRequest([string]$url,[string]$method,[int]$timeout,[int]$retry){
    for($i=0;$i -le $retry;$i++){
        ...
        try{
            ...
            if($method -in "POST","PUT","PATCH","PROPPATCH","MERGE"){
...
function Invoke-gRPC([string]$url,[int]$timeout,[int]$retry){
    for($i=0;$i -le $retry;$i++){
        ...
        try{
            ...
            $status=[int]$resp.StatusCode
            $resp.Close()
            return @{Url=$url;Type="gRPC";Result="OK";Status=$status;Latency=$sw.ElapsedMilliseconds}
...
foreach($ep in $endpoints){
    if($ConcurrencyMode -eq "Parallel"){
        $jobs += Start-Job -ScriptBlock {
            param($ep,$Timeout,$RetryCount,$HttpMethod)
            switch($ep.Type){
        ...
        } -ArgumentList $ep,$Timeout,$RetryCount,$HttpMethod
...
function sortTable(n){
    ...
    rows.sort(function(a,b){var x=a.cells[n].innerText.toLowerCase();var y=b.cells[n].innerText.toLowerCase();return asc?x.localeCompare(y):y.localeCompare(x);});
