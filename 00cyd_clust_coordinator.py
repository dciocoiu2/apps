...
class TerminalUI:
    ...
    def run(self, stdscr):
        ...
        while True:
            stdscr.clear()
            stdscr.addstr(0, 0, "CLUSTER COORDINATOR — Nodes:")
            row = 2

            for n in self.registry.all():
                stdscr.addstr(row, 0,
                              f"{n.id:17} {n.chip:10} {n.role:15} "
                              f"{n.transport:8} last={int(time.time()-n.last_seen)}s")
                row += 1
...
async def main():
    ...
    ui = TerminalUI(registry)
    loop = asyncio.get_event_loop()
    loop.run_in_executor(None, curses.wrapper, ui.run)
